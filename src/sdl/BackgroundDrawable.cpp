...
namespace jumpinjack
{
  ...
  BackgroundDrawable::BackgroundDrawable (
      SDL_Renderer * renderer, std::string imgfile, int parallax_level,
      bool repeat_x, int auto_speed) :
  ...
  void BackgroundDrawable::renderFixed (
      t_point point)
  {
    if (auto_speed)
      {
...
//          counter = 0;
    ...
    SDL_Rect renderQuad =
      { -(point.x/parallax_level % image_size.x), GlobalDefs::window_size.y - image_size.y, image_size.x, image_size.y };
    ...
    if (repeat_x)
      {
        renderQuad.x = -(point.x/parallax_level % image_size.x) + image_size.x;
        SDL_RenderCopyEx (renderer, mTexture, NULL, &renderQuad, 0, NULL,
                          SDL_FLIP_NONE);
      }
  }
